...
static int  __init exynos_set_debug_mem(void)
{
	...
	dcache_clean_poc((unsigned long)smc_debug_mem,
                 (unsigned long)smc_debug_mem + size);dcache_clean_poc((unsigned long)smc_debug_mem,
                 (unsigned long)smc_debug_mem + size);
				...
				unsigned int offset)
{
	...
	if (id) {
		...
		if ((offset > 0x0 && offset < (PAGE_SIZE * 2))
			...
			dcache_inval_poc((unsigned long)smc_debug_mem,
                 			 (unsigned long)smc_debug_mem + PAGE_SIZE * 2);
